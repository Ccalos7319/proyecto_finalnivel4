          ...
          <table className="w-full text-sm text-left text-gray-500 ">
            ...
            <tbody>
              {dataRoles.map((el) => (
                ...
                >
                  <td className="px-6 py-4">{el.idrol}</td>
                  <td className="px-6 py-4">{el.rol}</td>

                  <td className="flex items-center px-6 py-4 space-x-3">
                    <Link
                      to={`/edit roles/${el.idrol}`}
    ...
    <div className="px-10 flex items-center w-[80%] gap-5">
      ...
      <div className="flex py-5 justify-between items-center px-8">
        ...
        <form onSubmit={createRol} className="py-5 px-8">
          ...
          <button
            type="submit"
            className="mt-5 bg-gradient-to-r from-blue-500 to-blue-700 hover:from-blue-600 hover:to-blue-800 text-white font-bold py-3 px-6 rounded-full shadow-lg transform transition-all duration-500 ease-in-out hover:scale-110 hover:brightness-110 hover:animate-pulse active:animate-bounce"
          >
            Create
          </button>
        </form>
  ...
  useEffect(() => {
    const getRolById = async () => {
      const response = await axios.get(
        `http://127.0.0.1:8000/api/roles/${idrol}`
      );
    ...
    <div className="px-10 flex items-center w-[80%] gap-5">
      ...
      <div className="flex py-5 justify-between items-center px-8">
        ...
        <form onSubmit={update} className="py-5 px-8">
          ...
          <button
            className="bg-blue-500 hover:bg-blue-600 py-2 px-5 rounded-md text-white my-5"
            type="submit"
          >
            Save
          </button>
